    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(
            "windows95-active-border flex max-h-[min(16rem,var(--available-height))] max-w-(--available-width) min-w-32 origin-(--transform-origin) flex-col overflow-hidden bg-white outline-none",
            className
          )}
          {...props}
        />
...
function DropdownMenuLabel({
  ...
  return (
    <MenuPrimitive.GroupLabel
      ...
      data-inset={inset}
      className={cn(
        "windows95-text text-text/60 px-1.5 py-1 text-xs font-medium data-inset:pl-7",
        className
      )}
      {...props}
    />
...
function DropdownMenuSubTrigger({
  ...
  return (
    <MenuPrimitive.SubmenuTrigger
      ...
      data-inset={inset}
      className={cn(
        "windows95-text windows95-border text-text data-highlighted:bg-highlight data-popup-open:bg-highlight flex cursor-pointer items-center gap-1.5 px-1.5 py-1 text-sm outline-none select-none data-highlighted:text-white data-inset:pl-7 data-popup-open:text-white [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
      {children}
      <ChevronRightIcon className="cn-rtl-flip ml-auto size-3.5" />
    </MenuPrimitive.SubmenuTrigger>
  );
}
...
function DropdownMenuShortcut({ className, ...props }: ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        "text-text/60 ml-auto text-xs tracking-widest group-data-highlighted/dropdown-menu-item:text-white",
        className
      )}
      {...props}
    />
