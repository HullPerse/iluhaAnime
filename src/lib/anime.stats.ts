...
export function derivePersonalAnimeStats(lists: AniListCollection[]): PersonalAnimeStats {
  const entries = lists.flatMap((list) => list.entries);
  const count = (status: string) => entries.filter((entry) => entry.list_status === status).length;
  const scored = entries.filter((entry) => entry.score != null && entry.score > 0);
  ...
  return {
    ...
    episodesWatched: entries.reduce((total, entry) => total + (entry.progress ?? 0), 0),
    meanScore: scored.length
      ? scored.reduce((total, entry) => total + (entry.score ?? 0), 0) / scored.length
      : null,
    paused: count("PAUSED"),
    ...
    totalMinutes: entries.reduce(
      (total, entry) => total + (entry.progress ?? 0) * (entry.media.duration ?? 0),
      0
    ),
