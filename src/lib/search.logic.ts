...
export function filterAnimeResults(
  ...
  return data.filter((item) => {
    ...
    if (filters.quality !== "all" && !qualityMatch(item.title, filters.quality))
      return false;
    ...
    if (filters.sizeMin > 0 || filters.sizeMax > 0) {
      ...
      const minBytes = filters.sizeMin * 1_048_576;
      const maxBytes =
        filters.sizeMax > 0 ? filters.sizeMax * 1_048_576 : Infinity;
