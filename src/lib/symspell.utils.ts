...
export class SymSpell {
  ...
  suggest(query: string): string | null {
    // for multi-word query, correct each word separately
...
let globalSymSpell: SymSpell | null = null;

export function getGlobalSymSpell(): SymSpell {
  if (!globalSymSpell) globalSymSpell = new SymSpell();
  return globalSymSpell;
}
