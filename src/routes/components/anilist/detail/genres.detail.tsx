...
export function GenresTagsSection({
  ...
  const { t } = useI18n();
  return (
    <Section header={t("anilist.details.genres.tags")} className="bg-field flex flex-wrap gap-1">
      {genres.map((g) => (
        <Button
          key={g}
          onClick={() => {
          ...
          className="windows95-text bg-secondary hover:bg-secondary/60 windows95-active-border text-title-text flex flex-row gap-1 px-1 font-bold"
          variant="ghost"
