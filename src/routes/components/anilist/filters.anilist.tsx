...
function FiltersModal({
  ...
  const { close: handleDiscoveryBack } = discovery;
  const showDiscovery = discovery.open && discovery.current != null;
  ...
  return (
    <Modal
      header={t(showDiscovery ? "anilist.random.title" : "anilist.filters.title")}
      onClose={onClose}
      onBack={showDiscovery ? handleDiscoveryBack : undefined}
      className="w-xl"
    >
      {showDiscovery && discovery.current ? (
        <DiscoveryCard
          item={{ media: discovery.current, entry: entryLookup.get(discovery.current.id) }}
          nav={{
            index: discovery.index,
            total: discovery.total,
            canPrev: discovery.canPrev,
            busy: discovery.pending,
            onPrev: discovery.prev,
            onNext: discovery.reroll,
            onReroll: discovery.reroll,
            onDetails: onDiscoveryDetails,
          }}
        />
      ) : (
