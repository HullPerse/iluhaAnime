  ...
  const handleTestAllProxies = async () => {
    ...
    await Promise.all(
      sources.map(async (info) => {
        ...
        try {
          const res = await invoke<string>("test_source_connection", {
            source: info.value,
            proxyUrl: proxy || null,
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-1.5">
        ...
        <div className="flex flex-col gap-0.5">
          <Select
            ...
            className="w-28"
          />
          <span className="text-hint text-[12px]">{t("settings.search.default.source.hint")}</span>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-1">
            {SOURCE_INFOS.map((info) => (
              ...
              >
                ...
                />
                {info.nsfw && <span className="text-destructive text-xs font-bold">[NSFW]</span>}
                <span>{info.label}</span>
              </label>
            ))}
          </div>
        </div>
        <span className="text-hint text-[12px]">{t("settings.search.visible.sources.hint")}</span>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <span className="text-hint text-[12px]">{t("settings.search.proxy.per.source.hint")}</span>
            ...
            <div key={info.value} className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-0.5">
              ...
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center">
                ...
                {(() => {
                  ...
                  return (
                    <span
                      className={cn("windows95-text text-xs", st.ok ? "text-success" : "text-destructive")}
                    >
                      {st.ok
        ...
        {visibleSources.length === 0 && (
          <span className="text-hint text-[12px]">{t("settings.search.no.visible.sources")}</span>
        )}
      </div>
      ...
      >
        <span className="font-bold">{t("settings.search.fastembed.info.title")}</span> {t("settings.search.fastembed.info.description")}
      </div>
      ...
      <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-1.5">
        ...
        <div className="flex flex-col gap-0.5">
          <Select
            ...
            className="w-40"
          />
          <span className="text-hint text-[12px]">{t("settings.search.anilist.boost.hint")}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-1.5">
        ...
        <div className="flex flex-col gap-0.5">
          <Input
            ...
            className="w-16"
          />
          <span className="text-hint text-[12px]">{t("settings.search.anilist.max.pages.hint")}</span>
        </div>
      </div>
    </div>
  );
...
function FastembedSection() {
  ...
  useEffect(() => {
    invoke<boolean>("check_fastembed")
